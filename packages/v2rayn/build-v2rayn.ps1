$ROOT = git rev-parse --show-toplevel
. $ROOT/scripts/util.ps1


New-Item $env:PREFIX/bin/$name -ItemType Directory
if ($IsWindows) {
    gh release download -R 2dust/v2rayN -p "v2rayN-windows-64-SelfContained.zip" `
        -O  ./$name.zip --clobber
}
# if ($IsMacOS) {
#     gh release download -R 2dust/v2rayN -p "v2rayN-macos-arm64.zip" `
#         -O  ./$name.zip --clobber
# }
# if ($IsLinux -and $arch -eq "X64") {
#     gh release download -R 2dust/v2rayN -p "v2rayN-linux-64.zip" `
#         -O  ./$name.zip --clobber
# }
# if ($IsLinux -and $arch -eq "Arm64") {
#     gh release download -R 2dust/v2rayN -p "v2rayN-linux-arm64.zip" `
#         -O  ./$name.zip --clobber

# }
7z x "./$name.zip" "-o./$name"
if ($IsWindows) {
    Copy-Item "./$name/$name/v2rayN-windows-64-SelfContained/*" "$env:PREFIX/bin/$name/" -Recurse
}
# if ($IsMacOS) {
#     Copy-Item "./$name/$name/v2rayN-macos-arm64/*" "$env:PREFIX/bin/$name/" -Recurse
# }
# if ($IsLinux -and $arch -eq "X64") {
#     Copy-Item "./$name/$name/v2rayN-linux-64/*" "$env:PREFIX/bin/$name/" -Recurse
# }
# if ($IsLinux -and $arch -eq "Arm64") {
#     Copy-Item "./$name/$name/v2rayN-linux-arm64/*" "$env:PREFIX/bin/$name/" -Recurse
# }
# shortcut
New-Item $env:PREFIX/Menu -ItemType Directory
Copy-Item "$name.json" "$env:PREFIX/Menu"
