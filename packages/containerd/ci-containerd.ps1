...
if ($IsWindows) {
    gh release download -R "containerd/nerdctl" -p "$name-*-windows-amd64.tar.gz" `
        -O  $ROOT/temp/$name/$name.tar.gz --clobber
}
if ($IsLinux -and $arch -eq "X64") {
    gh release download -R "containerd/nerdctl" -p "$name-static-*.*.*-linux-amd64.tar.gz" `
        -O  $ROOT/temp/$name/$name.tar.gz --clobber
}
if ($IsLinux -and $arch -eq "Arm64") {
    gh release download -R "containerd/nerdctl" -p "$name-static-*.*.*-linux-arm64.tar.gz" `
        -O  $ROOT/temp/$name/$name.tar.gz --clobber
}
