...
function update-recipe {
    ...
    if ($env:CI) {
        ...
        switch ($true ) {
            ...
            { (-not $HAS_NEW_VERSION) -and ($env:GITHUB_EVENT_NAME -eq "push" ) -and ($env:GITHUB_REF_NAME -eq "main") } { pre-build -name $name; $env:PUBLISH = $true; "action_publish=true" >> $env:GITHUB_OUTPUT }
